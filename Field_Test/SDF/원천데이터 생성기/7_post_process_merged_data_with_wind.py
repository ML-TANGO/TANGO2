...
# 7_post_process_merged_data.py 와 동일하되, Out_anemometer_wind_speed / Out_anemometer_wind_direction 을 유지한다.
...
DEFAULT_OUTPUT = (
    "7_merged_data_postprocessed/time_sync_valid_data_merged_no_na_post_with_wind.csv"
)
...
def post_process_qa_dataframe(
    ...
    *,
    ...
    return replace_facility_with_metadata(
        df,
        facility_meta,
        temp_col=temp_col,
        growing_degree_days_base_c=growing_degree_days_base_c,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "QA CSV (풍속/풍향 유지), facility_id 대신 "
