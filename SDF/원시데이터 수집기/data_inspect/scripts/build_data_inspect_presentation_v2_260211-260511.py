#!/usr/bin/env python3
"""Improved presentation deck from SDL master reference."""
...
PPTX_PATH = PRESENTATION_ROOT / "data_inspect_260211-260511_presentation_v2.pptx"

# Reuse SDL master slide dimensions
SLIDE_W = Inches(13.333)
SLIDE_H = Inches(7.5)
# SDL used Noto Sans KR
FONT = "Noto Sans KR"
...
ACCENT_LINE = RGBColor(220, 226, 232)

PRESENTATION_ROOT.mkdir(parents=True, exist_ok=True)
PLOT_ROOT.mkdir(parents=True, exist_ok=True)

plt.rcParams["axes.unicode_minus"] = False
# Use an available CJK font for matplotlib plots; PPTX text uses FONT separately
for _plot_font in ["Apple SD Gothic Neo", "AppleGothic", "NanumGothic", "DejaVu Sans"]:
    try:
...
def calculate_summary() -> dict:
    ...
    if not joined.empty:
        monthly = joined.groupby("month", as_index=False).agg(
        ...
        monthly["open_temp_corr"] = joined.groupby("month").apply(lambda x: x["mean_open_rate"].corr(x["internal_temp"]), include_groups=False).values
...
def create_summary_plots(summary: dict) -> dict[str, Path]:
    ...
    if not monthly.empty:
        path = PLOT_ROOT / "monthly_open_rate_temperature_summary_v2.png"
    ...
    # NEW: monthly open_rate boxplot (replaces scatter)
    if not open_rate.empty:
        open_rate["month"] = pd.to_datetime(open_rate["local_ts"], errors="coerce").dt.strftime("%Y-%m")
        months = sorted(open_rate["month"].dropna().unique())
        data = [open_rate.loc[open_rate["month"] == month, "open_rate"].dropna() for month in months]
        path = PLOT_ROOT / "monthly_open_rate_boxplot.png"
        fig, ax = plt.subplots(figsize=(9.8, 5.6))
        bp = ax.boxplot(data, tick_labels=months, showfliers=False, patch_artist=True)
        for patch in bp["boxes"]:
            patch.set_facecolor("#aec7e8")
            patch.set_alpha(0.7)
        ax.set_ylabel("개폐율(%)")
        ax.set_title("월별 개폐율 분포: 4~5월이 상단에 집중되어 있음")
        ax.set_ylim(-5, 105)
        fig.tight_layout()
        fig.savefig(path, dpi=160)
        plt.close(fig)
        plots["open_rate_boxplot"] = path
...
def make_deck(summary: dict, plots: dict[str, Path]) -> None:
    ...
    # Slide 4: Temperature extraction results
    slide = prs.slides.add_slide(blank)
    ...
    # Slide 5: April-May core issue (SCATTER REPLACED)
    slide = prs.slides.add_slide(blank)
    ...
    # Left: monthly open_rate boxplot (shows distribution per month, no direct temp correlation)
    add_image(slide, plots.get("open_rate_boxplot"), Inches(0.75), Inches(3.0), Inches(5.95), Inches(3.65))
    # Right: temp boxplot + text emphasis (replacing scatter)
    add_image(slide, plots.get("temp_boxplot"), Inches(6.95), Inches(3.0), Inches(5.65), Inches(3.65))

    # Slide 6: B notebook (REDUCED graphs, ADDED text)
    slide = prs.slides.add_slide(blank)
    add_title(slide, "5. B 노트북: 실제 open_rate와 온도 추세", "이벤트 수만이 아니라 실제 개폐율 값의 전체 기간 변화를 확인")
    # Larger single graph instead of two small ones
    add_image(slide, B_OUTPUT / "plots" / "daily_open_rate_temperature_trend.png", Inches(0.7), Inches(1.45), Inches(7.2), Inches(5.45))
    # Text explanation replacing the second graph
    add_bullets(
        slide,
        [
            "일별 평균 개폐율(파란 막대)과 내부/외부온도(선)를 동시에 확인.",
            "2월: 낮은 개폐율 + 저온 구간이 충분히 관측됨.",
            "3월: 개폐율이 점진적으로 상승하며 온도도 상승.",
            "4월 이후: 대부분의 채널이 지속적으로 높은 개폐율 상태.",
            "채널별 일별 추세 그래프는 생략하고 핵심 메시지만 요약: 모든 채널이 4월부터 급격히 상승하여 이후 대부분 90% 이상을 유지.",
            "이 구조에서는 '온도 상승 → 개폐율 증가'를 명확히 분리할 수 없음.",
        ],
        Inches(8.15), Inches(1.45), Inches(4.75), Inches(5.45), 15,
    )
    ...
    # Slide 7: Basic correlation
    slide = prs.slides.add_slide(blank)
    add_title(slide, "6. 기본 상관 분석: 온도가 가장 강한 신호로 관측", "5분 리샘플링 기준 개폐율-환경변수 상관")
    add_image(slide, BASE_OUTPUT / "plots" / "open_rate_environment_correlation_heatmap.png", Inches(0.7), Inches(1.45), Inches(6.2), Inches(5.1))
    add_table(slide, top_base, Inches(7.1), Inches(1.45), Inches(5.55), Inches(3.1), 8)
    add_bullets(
        slide,
        [
        ...
        Inches(7.15), Inches(4.75), Inches(5.5), Inches(1.8), 14,
    )

    # Slide 8: C notebook (REDUCED graphs)
    slide = prs.slides.add_slide(blank)
    add_title(slide, "7. C 노트북/스크립트: 주 단위 기상·온도 상세 분석", "전체 기간을 한 번에 보지 않고 7일 단위로 분해")
    # Single larger overview image
    add_image(slide, C_OUTPUT / "week_10_20260422" / "week_overview_open_rate_temperature_rain_solar.png", Inches(0.65), Inches(1.35), Inches(7.2), Inches(5.45))
    # Replace heatmap with text emphasis
    add_bullets(
        slide,
        [
            "대표 예시: 2026-04-22 주차 (week 10).",
            "개별 채널 open_rate와 내부/외부온도, 강우, 일사/일조를 같은 축에서 확인.",
            "이 기간에는 낮 시간대 대부분의 채널이 90% 이상 개방.",
            "주 단위 상관 히트맵은 생략하고 핵심만 요약: 채널별로 온도-개폐율 상관 크기가 다르며, 일사/일조와 온도가 함께 작용.",
            "특히 4~5월 주차에서는 상관 계수가 높게 나타나지만, 이는 데이터 구조상 편향으로 인한 결과일 수 있으므로 인과 해석에는 주의가 필요하다.",
            "주 단위 산출물은 스크립트로 전체 13주를 일괄 생성하도록 구성했다.",
        ],
        Inches(8.05), Inches(1.35), Inches(4.85), Inches(5.45), 15,
    )
    ...
    # Slide 9: Weekly weather summary (kept table, no extra graph)
    slide = prs.slides.add_slide(blank)
    add_title(slide, "8. 주 단위 기상 분석 요약", "채널별로 온도·강우·일사 변수와의 관련성이 다르게 나타남")
    add_table(slide, top_weather, Inches(0.75), Inches(1.45), Inches(6.5), Inches(4.5), 8)
    add_bullets(
        slide,
        [
            ...
            "주 단위 산출물은 스크립트로 전체 13주를 일괄 생성하도록 구성했다.",
            "상관이 높은 주차라도 개폐율이 이미 대부분 열려 있으면, 이를 온도-개폐율 인과로 해석하면 안 된다.",
        ],
        Inches(7.45), Inches(1.45), Inches(5.2), Inches(4.5), 15,
    )

    # Slide 10: D notebook (REDUCED graphs)
    slide = prs.slides.add_slide(blank)
    add_title(slide, "9. D 노트북/스크립트: 습도와 개폐율 관계", "내부/외부/KMA 습도와 lag 후보를 별도로 확인")
    add_image(slide, D_OUTPUT / "week_10_20260422" / "week_open_rate_humidity_overview.png", Inches(0.65), Inches(1.35), Inches(7.2), Inches(5.45))
    add_bullets(
        slide,
        [
            "대표 예시: 2026-04-22 주차 (week 10).",
            "개별 채널 open_rate와 내부습도, 외부습도, KMA 습도를 함께 확인.",
            "습도 lag 분석 결과는 채널별로 다르며, 대부분 동시성(lag=0)이 가장 높게 나타남.",
            "습도는 강우·온도 변화와 결합된 결과이므로 독립적인 개폐율 예측 변수로 보기 어렵다.",
            "lag 히트맵은 생략하고 핵심만 요약: 대부분 채널에서 습도-개폐율 lag 상관은 즉각적이거나 약하다.",
            "모델에는 습도를 독립 feature가 아닌, 온도·강우와 결합한 복합 feature로 활용하는 것이 안전하다.",
        ],
        Inches(8.05), Inches(1.35), Inches(4.85), Inches(5.45), 15,
    )
    ...
    # Slide 11: Humidity summary
    slide = prs.slides.add_slide(blank)
    add_title(slide, "10. 습도 분석 요약", "습도 자체보다 온도·강우와 결합한 feature로 보는 것이 안전")
    add_table(slide, top_humidity, Inches(0.75), Inches(1.45), Inches(6.6), Inches(4.5), 8)
    add_bullets(
        slide,
        [
            ...
            "습도 lag 히트맵은 채널별로 해석이 제한적이어서, 주 단위 개요 그래프 위주로 요약했다.",
        ],
        Inches(7.55), Inches(1.45), Inches(5.1), Inches(4.5), 15,
    )
    ...
    add_bullets(
        ...
        [
            ...
            "PPT 생성 스크립트: data_inspect/scripts/build_data_inspect_presentation_v2_260211-260511.py",
            f"PPT 파일(v2): {PPTX_PATH.relative_to(PROJECT_ROOT)}",
