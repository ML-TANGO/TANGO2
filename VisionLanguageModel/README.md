...
SDS(Software-defined Ship) 해상 도메인 데이터셋에 특화된 3단계 학습 파이프라인을 제공합니다.
...
| Phase 2a | `train_lora.sh` | CC3M 595K | Projector + LLM(LoRA) | CLIP | 일반 시각-언어 파인튜닝 |
| Phase 2b | `train_lora_marine.sh` | LLaMarine 54K (텍스트 전용) | LLM(LoRA) 계속학습 | 전부 | 해양 도메인 지식 주입 |
| Phase 3 | `train_lora_sds.sh` | SDS 100개 (이미지+텍스트) | Projector + LLM(LoRA) 계속학습 | CLIP | SDS 태스크 특화 |

> Phase 2b는 이미지 없이 텍스트만 사용하므로 Vision Encoder / Projector를 로드하지 않습니다.
...
| 항목 | 권장 |
|------|------|
| GPU | NVIDIA RTX 5090 (32 GB) 또는 A100-40G × N |
| VRAM (Phase 1) | 24 GB 이상 (batch 8 기준) |
| VRAM (Phase 2/3 LoRA) | 24 GB 이상 |
| CUDA | 12.8 |
...
```
┌─────────────────────────────┬──────────────────────────┐
│  📸 입력 이미지 (bbox 오버레이)│  📁 데이터셋 탐색기        │
│  📊 AIS 데이터 테이블         │  - 경로 입력 + 📂 탐색    │
│                             │  - 스캔 → 샘플 드롭다운   │
├─────────────────────────────┼──────────────────────────┤
│  📋 기대값 (Ground Truth)    │  ⚙️ 모델 설정             │
│  - 5가지 출력 유형 선택       │  - 체크포인트 선택 + 📂   │
│  - 레퍼런스 텍스트 표시       │  - 비전/LLM 경로 + 📂    │
│                             │  - 모델 로드 버튼         │
│                             │  💬 추론 결과 채팅         │
└─────────────────────────────┴──────────────────────────┘
```
...
│   └── run.sh                 # 데모 실행 스크립트
│
├── checkpoints/               # 학습 결과물 저장 위치
│   ├── clip_llama31_projector/    # Phase 1 결과
│   ├── clip_llama31_lora/         # Phase 2a 결과
│   ├── clip_llama31_lora_marine/  # Phase 2b 결과
│   ├── sds_lora_en/               # Phase 3 영문
│   ├── sds_lora_ko/               # Phase 3 한글
│   └── sds_lora_ko_compact/       # Phase 3 한글 간결
│
...
### `conda` 패키지가 `defaults`에서 다운로드됨

```bash
cat ~/.condarc                           # defaults 확인
conda config --remove channels defaults  # 제거
```

### `CUDA out of memory`

배치를 줄이고 누적 스텝을 늘립니다 (effective batch size 유지):
...
LoRA 학습 시 `_get_embed_layer()`가 PeftModel 래퍼를 통과하지 못할 때 발생합니다.  
`model/vlm_v2.py`의 `_get_embed_layer()`는 최대 4단계까지 `.model`을 재귀 탐색하도록 구현되어 있습니다.  
오류가 발생한다면 `vlm_v2.py`가 최신 버전인지 확인하십시오.

### Qwen3 응답이 비어 있음

Qwen3는 `<think>...</think>` 블록 이후에 실제 답변을 생성합니다.  
`test.py`와 `demo/app.py` 모두 `</think>` 이후 텍스트를 자동으로 추출합니다.  
응답이 비어 있다면 Phase 2 LoRA 파인튜닝이 충분히 이루어지지 않은 것입니다.
...
### Llama 3.1 다운로드 인증 오류

```bash
huggingface-cli login   # HF 토큰 입력
# Meta 공식 사이트에서 Llama 3.1 라이센스 동의 필요
```
