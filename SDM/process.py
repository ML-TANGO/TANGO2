...
def find_best_matches_two_stage(image_features, findings_features, findings_texts, impression_features, impression_texts, logit_scale=None, logit_bias=None):
    """
    Two-stage retrieval: 
    1. Find top-1 findings
    2. Among impressions that share the same text as top-1 findings, find the best one
    """
    
    # Ensure all features are on the same device
    device = image_features.device
    if findings_features.device != device:
        findings_features = findings_features.to(device)
    if impression_features.device != device:
        impression_features = impression_features.to(device)
    
    # Normalize features
    image_features = F.normalize(image_features.float(), dim=-1)
    findings_features = F.normalize(findings_features.float(), dim=-1)
    impression_features = F.normalize(impression_features.float(), dim=-1)
    
    # Stage 1: Find best findings
    findings_logits = (image_features @ findings_features.t()).float()
    
    if logit_scale is not None:
        if logit_scale.device != device:
            logit_scale = logit_scale.to(device)
        findings_logits = logit_scale.mean().float() * findings_logits
    
    if logit_bias is not None:
        if logit_bias.device != device:
            logit_bias = logit_bias.to(device)
        findings_logits = findings_logits + logit_bias.float()
    
    best_findings_idx = torch.argmax(findings_logits, dim=1).cpu().numpy()[0]
    best_findings_text = findings_texts[best_findings_idx]
    best_findings_score = torch.max(findings_logits, dim=1)[0].cpu().numpy()[0]
    
    # Stage 2: Find impressions that share the same text as best findings
    matching_impression_indices = []
    for i, impression_text in enumerate(impression_texts):
        if impression_text == best_findings_text:
            matching_impression_indices.append(i)
    
    if matching_impression_indices:
        # Get features for matching impressions only
        matching_impression_features = impression_features[matching_impression_indices]
        
        # Compute similarity with matching impressions
        impression_logits = (image_features @ matching_impression_features.t()).float()
        
        if logit_scale is not None:
            impression_logits = logit_scale.mean().float() * impression_logits
        
        if logit_bias is not None:
            impression_logits = impression_logits + logit_bias.float()
        
        # Find best among matching impressions
        best_matching_idx = torch.argmax(impression_logits, dim=1).cpu().numpy()[0]
        best_impression_idx = matching_impression_indices[best_matching_idx]
        best_impression_text = impression_texts[best_impression_idx]
        best_impression_score = torch.max(impression_logits, dim=1)[0].cpu().numpy()[0]
        
    else:
        # Fallback: if no matching impressions found, use original method
        impression_logits = (image_features @ impression_features.t()).float()
        ...
        best_impression_idx = torch.argmax(impression_logits, dim=1).cpu().numpy()[0]
        best_impression_text = impression_texts[best_impression_idx]
        best_impression_score = torch.max(impression_logits, dim=1)[0].cpu().numpy()[0]
    
    return (best_findings_text, best_impression_text), (best_findings_score, best_impression_score), (best_findings_idx, best_impression_idx)

def generate_reports(model, tokenizer, input_dir, impressions_path, findings_path, device="cuda", precision="bf16", use_3channel=False, separator="!@#$%^&*()", impressions_emb=None, findings_emb=None, two_stage_retrieval=False):
    ...
    with torch.no_grad():
        for img_path in tqdm(image_files, desc="Processing CT volumes"):
            ...
            try:
                ...
                if two_stage_retrieval:
                    # Use two-stage retrieval: first find best findings, then find best impression among matching ones
                    (best_findings_text, best_impression_text), (findings_score, impression_score), (findings_idx, impression_idx) = find_best_matches_two_stage(
                        image_features, findings_features, findings, impression_features, impressions,
                    ...
                    # Format report
                    report = f"Findings: {best_findings_text} Impression: {best_impression_text}"
                    ...
                    logging.info(f"Processed {input_image_name}: "
                               f"Findings score: {findings_score:.4f}, "
                               f"Impression score: {impression_score:.4f}")
                else:
                    # Use original method: find best impression and findings independently
...
def main():
    ...
    parser.add_argument("--two_stage_retrieval", action="store_true",
                       help="Use two-stage retrieval: first find best findings, then find best impression among matching ones")
    ...
    results = generate_reports(
        ...
        two_stage_retrieval=args.two_stage_retrieval
    )
