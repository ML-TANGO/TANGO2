...
class BaseDataset(d.Dataset):
    def __init__(self, data_path1: str, data_path2: str, model_id: str):
        super().__init__()
        
        dataset1 = self._load_data(data_path1)
        dataset2 = self._load_data(data_path2)
        random.shuffle(dataset2)
        dataset2 = dataset2[:len(dataset1)] if len(dataset1) < len(dataset2) else dataset2
