...
"""
Llama 3.1 8B Instruct + FIPO 형식 DPO 학습 스크립트

사용 예시:
python /scratch/x3397a11/minkyu/workspace/ETRI/prompt_optimization/prompt_optimization_FIPO/codes/train_dpo_llama_fipo.py \
    --base_model_name_or_path meta-llama/Meta-Llama-3.1-8B-Instruct \
    --sft_adapter_path /scratch/x3397a11/minkyu/workspace/ETRI/prompt_optimization/model/FIPO_sft_llama3_1_8b_instruct/checkpoint-5625 \
    --output_dir /scratch/x3397a11/minkyu/workspace/ETRI/prompt_optimization/model/dpo/dpo_llama3_1_8b_use_silver \
    --hf_token $HF_TOKEN
"""
...
DEFAULT_OUTPUT_DIR = (
    "/scratch/x3397a11/minkyu/workspace/ETRI/prompt_optimization/model/dpo/"
    "dpo_llama3_1_8b_use_silver"
)


@dataclass
class LlamaDPOTrainingArguments(DPOConfig):
...
class ModelArguments:
    model_name_or_path: str = field(
        default="/scratch/x3397a11/minkyu/workspace/ETRI/prompt_optimization/model/FIPO_sft_llama3_1_8b_instruct/checkpoint-5625"
    )
    base_model_name_or_path: str = field(default="meta-llama/Meta-Llama-3.1-8B-Instruct")
    ...
    hf_token: Optional[str] = field(default=os.environ.get("HF_TOKEN", None))
...
class DataArguments:
    ...
    prompt_template_path: str = field(
        default="/scratch/x3397a11/minkyu/workspace/ETRI/prompt_optimization/prompt_optimization_FIPO/data/prompts.json"
    )
...
def build_optimizer_prompt(tokenizer, optimizer_text: str, use_chat_template: bool = True) -> str:
    ...
    if use_chat_template and hasattr(tokenizer, "apply_chat_template"):
        ...
        # Qwen tokenizer만 enable_thinking 파라미터를 지원
...
def main():
    parser = HfArgumentParser((ModelArguments, DataArguments, LlamaDPOTrainingArguments))
    model_args, data_args, training_args = parser.parse_args_into_dataclasses()
    set_seed(training_args.seed)

    if model_args.hf_token:
        os.environ["HF_TOKEN"] = model_args.hf_token
    ...
    print(f"[DPO output] {training_args.output_dir}")
    print(
