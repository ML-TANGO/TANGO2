...
class MyTrainingArguments(TrainingArguments):
    output_dir: str = field(
        default="/scratch/x3397a11/minkyu/workspace/ETRI/prompt_optimization/model/FIPO_sft_llama3_1_8b_instruct"
    )
...
class DataArguments:
    ...
    fallback_max_words: int = field(default=512)
    context_mix_mode: str = field(
        default="both",
        metadata={"help": "none|silver|golden|both|mixed"},
    )
...
def main():
    ...
    rng = random.Random(data_args.context_mix_seed)
    source_texts = []
    targets = []
    ...
    for ex in ds:
        ...
        silver_text = get_optional_value(ex, data_args.silver_response_column) if use_silver else None
        golden_text = get_optional_value(ex, data_args.golden_response_column) if use_golden else None
