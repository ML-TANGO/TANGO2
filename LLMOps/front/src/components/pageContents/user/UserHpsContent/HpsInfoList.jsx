...
import style from './HpsInfoList.module.scss';
  ...
  const deleteToolConfirmPopup = () => {
    dispatch(
      openConfirm({
        ...
        notice: t('deleteHPSPopup.notice.message'),
        contentCustomStyle: {
    ...
    <div className={cx('container')}>
      <div className={cx('basic-info', isOpen && 'open')}>
        ...
        <div className={cx('info-box')}>
          ...
          <div className={cx('detail')}>
            <span className={cx('type')}>도커 이미지</span>
          ...
          <div className={cx('detail')}>
            <span className={cx('type')}>실행 코드</span>
        ...
        <div className={cx('detail-info')}>
          ...
          <div className={cx('info-box')}>
            ...
            <div className={cx('detail', 'long-detail')}>
              <span className={cx('type')}>검색 파라미터</span>
              <div className={cx('value', 'paramValue')}>
                {parameter.map(({ name, max, min, step }, index) => (
                  <span className={cx('param')} key={index}>
                    <span className={cx('name')}>{name}</span>
                    <span className={cx('param-info')}>
                      <span className={cx('first-text')}>최솟값</span>
                      <span className={cx('second-text')}>{min}</span>
                    </span>
                    <span className={cx('param-info')}>
                      <span className={cx('first-text')}>최대값</span>
                      <span className={cx('second-text')}>{max}</span>
                    </span>
                    <span className={cx('param-info')}>
                      <span className={cx('first-text')}>검색 횟수</span>
                      <span className={cx('second-text')}>{step}</span>
                    </span>
                  </span>
                ))}
              </div>
