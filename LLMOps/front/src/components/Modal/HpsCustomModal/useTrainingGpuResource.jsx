...
const useTrainingGpuResource = ({ trainId }) => {
  ...
  const fetchGpuClusterType = useCallback(async () => {
    if (gpuInfo.used > 1 && gpuClusterSelectedOption === 1) {
      const response = await callApi({
        url: `projects/gpu-cluster-auto?project_id=${trainId}&gpu_count=${gpuInfo.used}`,
        method: 'GET',
      });
      const { status, result } = response;
  ...
  }, [gpuInfo, gpuClusterSelectedOption, trainId]);
...
export default useTrainingGpuResource;
