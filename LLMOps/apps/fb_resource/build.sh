...
if [[ -z "$IMAGE_INFO" ]]; then
    ...
    echo "Build the first version of image. ($IMAGE_REGISTRY/$APP_NAME:0.2.1)"
    ...
    sudo nerdctl build --no-cache --tag $IMAGE_REGISTRY/$APP_NAME:0.2.1 -f $DOCKERFILE ..
    sudo nerdctl push $IMAGE_REGISTRY/$APP_NAME:0.2.1
