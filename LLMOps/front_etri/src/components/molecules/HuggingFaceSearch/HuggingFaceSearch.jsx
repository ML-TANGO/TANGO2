...
const HuggingFaceSearch = ({
  ...
  const fetchCategory = async () => {
    const response = await callApi({
      url: 'options/huggingface/category',
    });
  ...
  }, [huggingFaceToken, keyword, scope.value, selectCategory.value]);
  ...
  useEffect(() => {
    fetchModel();
  }, [keyword, scope, selectCategory, fetchModel]);
    ...
    <div className={cx('container')} {...rest}>
          ...
          <div className={cx('data-box')}>
            {categoryList.map(({ name }, index) => (
              ...
              >
                <div className={cx('left-side')}>
                  <img
                    src={`/images/icon/hugging/${name}.svg`}
                    width={16}
                    height={16}
                    alt='icon'
                  />
      ...
      {isModelOpen && (
        <div className={cx('content-box', 'data', isModelOpen && 'open')}>
          ...
          <div className={cx('data-box', visibleToolTip && 'pb-gap')}>
