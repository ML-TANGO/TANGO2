...
class ModelArguments:
    ...
    trust_remote_code: bool = field(default=True, metadata={"help": "Enable remote code for Qwen models."})
    lora_r: int = field(default=16, metadata={"help": "LoRA rank."})
    lora_alpha: int = field(default=32, metadata={"help": "LoRA alpha."})
    lora_dropout: float = field(default=0.05, metadata={"help": "LoRA dropout."})
    if_lora: int = field(default=1, metadata={"help": "1: LoRA, 0: full fine-tuning."})
...
def build_source(raw_prompt: str) -> str:
    return (
...
def main():
    ...
    def preprocess_function(examples):
        ...
        labels_list = []
        attention_masks = []
        for raw_prompt, target in zip(examples[data_args.raw_prompt_column], examples[data_args.target_column]):
            source_text = build_source(raw_prompt)
            target_text = target + eos_token
            source_ids = tokenizer.encode(source_text, add_special_tokens=False)
            target_ids = tokenizer.encode(target_text, add_special_tokens=False)
            input_ids = (source_ids + target_ids)[: data_args.model_max_length]
            labels = ([label_ignore_id] * len(source_ids) + target_ids)[: data_args.model_max_length]
