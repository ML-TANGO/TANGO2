...
async def get_records_option_workspaces():
    ...
    """
    cr = CustomResource()
    res = svc.options_workspaces(headers_user=cr.check_user())
    return res
...
async def get_records_option_trainings(
    ...
    """
    cr = CustomResource()
    res = svc.options_trainings(workspace_id=workspace_id, headers_user=cr.check_user())
    return res
"""
...
async def get_records_history(
    ...
    """
    res = svc.records_history(page=page, size=size, sort=sort, order=order, action=action, task=task)
    return res
...
async def get_utilization_figure(
    ...
    aggregation: str = Query("minute", description="집계 단위, minute/hour/day/week/month/quarter/year 중 하나. 주석: 기간 및 화면 너비에 따라 프론트에서 결정해서 보내주시면 좋겠는데, 불가능하시면 말씀주세요.")
):
    """
