...
import traceback
from utils.common import byte_to_gigabyte, convert_unit_num
...
def get_instance_list(node_id):
    ...
    try:
        ...
        cpu_info = redis_client.hget(redis_key.CPU_INFO_RESOURCE, node_info['name'])
        if cpu_info:
            free_cpu = json.loads(cpu_info)['cpu_core']
        # node_cpu = db_node.get_node_cpu(node_id=node_id)['core']

        mem_info = redis_client.hget(redis_key.MEM_INFO_RESOURCE, node_info['name'])
        if mem_info:
            free_ram=byte_to_gigabyte(json.loads(mem_info)['mem_total'])
        # node_ram = 0
        ...
        for item in db_node.get_node_gpu_list(node_id=node_id, group_by_gpu=True):
            node_gpu_list[item["resource_group_id"]] = {
                ...
                "vram" :  convert_unit_num(f'{item["gpu_memory"]}M', 'G', True)  , # 같은 모델인데 메모리 다를 수가 있는지???
            }
        ...
        node_npu_list = dict()
        try:
            for item in db_node.get_node_npu_list(node_id=node_id, group_by_gpu=True):
                node_npu_list[item["resource_group_id"]] = {
                    "name" : item["resource_name"],
                    "group_id" : item["resource_group_id"],
                    "total" : item["npu_count"],
                }
        except:
            pass
        node_npu_id_list = list(node_npu_list.keys())
        result_node_npu_list = node_npu_list
        ...
        free_npu_id_list = node_npu_id_list
        ...
        last_free_resource = {
            ...
            "npu_id_list" : free_npu_id_list,
        }
        #======================================================================================================
        # Result
        result = {
            "node_gpu_list" : result_node_gpu_list,
            "node_npu_list" : result_node_npu_list,
...
def update_instance_list(node_id, instance_list):
    ...
    instance_list = [{
        ...
        "instance_name" : str,
        "instance_type" : str,
        "instance_count" : int, -> 이전에 유효성 검사를 한 인스턴스는 count가 채워져서 전달됨
    }, {}, {}, ...]
    """
    try:
        # node 정보 ==================================================================
        ...
        for item in instance_list:
            # 최초로 만들어지는 인스턴스는 인스턴스 아이디가 없음 -> 이름으로 있는지 조회
            ...
            if item_instance_name in instance_db_name_list:
                ...
                if item_instance_name in node_instance_db_name_list:
                    # 인스턴스 수정 (DB instance 수정, node_instance 수정)
                    ...
                    # 인스턴스 수정 (DB instance 수정, node_instance 생성) - 같은 이름의 인스턴스를 다른 노드에서 최초로 생성하는 경우
                    update_list.append({
                        "instance_id" : item_instance_id,
                        "instance_name" : item_instance_name,
        ...
        db_instance.init_workspace_instance(instance_id_list=init_instance_id_list)
...
def get_instance_validation(node_id: int, instance_list: list):
    """
    input instance_list는 웹에서 유효성 검사를 요청한 인스턴스 리스트
    유효성 검사 누른 인스턴스만 보내는게 아니라, 모달창에서 선택한 인스턴스를 다 보냄

    instance_list = [{
        "cpu_allocate" : int,
        "gpu_allocate" : int,
        "ram_allocate" : int,
        "gpu_group_id" : int, -> TODO key resource_group_id 변경
        "instance_name" : str,
        "instance_type" : str,
        "instance_count" : int, -> 이전에 유효성 검사를 한 인스턴스는 count가 채워져서 전달됨
    }, {}, {}, ...]
    ...
    try:
        ...
        node_gpu_id_list = list(node_gpu_list.keys())
        result_node_gpu_list = node_gpu_list # Return 값에 내려줄 전체 gpu 리스트
        free_gpu_id_list = node_gpu_id_list # 잔여 GPU 리스트
        ...
        for item in instance_list: 
            ...
            if instance_count is not None:
                # 2-1. 이미 개수가 할당된 경우 (이미 유효성 검사를 한 인스턴스)
                # free check
                ...
                if item["instance_type"] == INSTANCE_TYPE_GPU:
                    free_gpu_id_list.remove(item["gpu_group_id"]) # 유효성 검사로 통과된 인스턴스는 gpu 제거
                    
            else:
                # 2-2. 아직 유효성 검사를 하지 않은 인스턴스
                def safe_divide(a, b):
                    try:
                        return a // b
                ...
                # (잔여량 / 할당량) 했을때 몇 개까지 만들 수 있는지 체크
                # ex) CPU: 16 core / 4 core = 4, CPU만 계산했을때는 인스턴스 4개 만들수 있다.
                ...
                if 0 in check_list:
                    # 유효성 검사 실패
                    # cpu, ram, gpu 중 하나라도 0보다 작으면 인스턴스를 만들수 없음
                    ...
                    if check_ram <= 0:
                        instance_validation_message += f"할당한 ram({ram_allocate})이 잔여 ram({free_ram}) 초과함,"
                    ...
                    # 체크리스트에서 최소값으로 인스턴스 개수를 정함 [cpu 4, ram 2] -> 인스턴스 2개 만듬
                    instance_validation_message = None
                
                # instance list (유효성 검사를 마치고 인스턴스 추가)
                node_instance_info_list.append({
                    "instance_name" : item["instance_name"],
                    "instance_type" : item["instance_type"],
