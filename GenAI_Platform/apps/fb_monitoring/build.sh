...
if [[ -z "$IMAGE_INFO" ]]; then
    ...
    echo "Build the first version of image. ($IMAGE_REGISTRY/$APP_NAME:$IMAGE_VERSION)"
    ...
    sudo docker build --no-cache --tag $IMAGE_REGISTRY/$APP_NAME:$IMAGE_VERSION -f $DOCKERFILE ..
    sudo docker push $IMAGE_REGISTRY/$APP_NAME:$IMAGE_VERSION
