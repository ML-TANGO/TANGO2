...
async def get_projest_by_instance_id(instance_id : int, workspace_id : int):
    sql = """
        SELECT *
        FROM project
        WHERE workspace_id = %s AND instance_id = %s
...
async def update_project_new(project_id : int, project_name : str, access : int, description : str, instance_id: int, instance_allocate: int, owner_id : int):
    params = [project_name, access, description, instance_id, instance_allocate, owner_id, project_id]
    sql = """
        UPDATE project SET
        name = %s,
        access = %s,
...
def update_project(project_id : int, project_name : str, access : int, description : str, instance_id: int, instance_allocate: int, owner_id : int):
    try:
        with get_db() as conn:
            cur = conn.cursor()
...
def update_project_tool_request(project_tool_id : int, request_status : int, gpu_count : int, gpu_cluster_auto : bool, image_id : int, gpu_select : str, tool_password : str):
    try:
...
def update_project_tool_datetime(project_tool_id : int, start_datetime: str = None, end_datetime : str = None):
    try:
        with get_db() as conn:
            cur = conn.cursor()
            sql = """
...
def update_project_training_datetime(training_type :str ,training_id : int, start_datetime: str = None, end_datetime : str = None):
    try:
        with get_db() as conn:
            cur = conn.cursor()
            sql = f"""
...
async def check_duplicate_training(project_id :int , training_name : str):
    ...
    sql = f"""
        SELECT *
        FROM training
        WHERE name = {training_name} AND project_id = {project_id}
        """
    res = await select_query(sql, fetch_type="one")
    return res
    ...
    try:
        with get_db() as conn:
            ...
            sql = f"""
                SELECT *
                FROM project
                WHERE name = {project_name} AND workspace_id = {workspace_id}
                """
            cur.execute(sql)
            res = cur.fetchone()
        return res
    ...
    try:
        with get_db() as conn:
            ...
            sql = f"""
                SELECT t.*
                FROM training t
                WHERE t.project_id = {project_id} AND start_datetime IS NOT NULL AND end_datetime IS NULL
                """
            cur.execute(sql)
            res = cur.fetchall()
        return res
