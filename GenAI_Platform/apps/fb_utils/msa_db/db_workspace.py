...
def insert_workspace(manager_id:int, workspace_name:str, start_datetime:str, end_datetime:str, description:str, path:str, data_storage_size:str, main_storage_size:str, data_storage_id: int, main_storage_id: int):
    try:
        with get_db() as conn:
            cur = conn.cursor()

            fields = ['manager_id', 'name','start_datetime', 'end_datetime', 'description', 'path', 'data_storage_size', 'main_storage_size', 'data_storage_id','main_storage_id']
            sql = """INSERT INTO {} ({})
                VALUES ({})""".format('workspace', ', '.join(fields), ', '.join(['%s']*len(fields)))

            cur.execute(sql, (manager_id, workspace_name, start_datetime, end_datetime, description, path, data_storage_size, main_storage_size, data_storage_id,main_storage_id))
    ...
    try:
        ...
        with get_db() as conn:
            ...
            SELECT (SELECT COUNT(*) FROM datasets d WHERE d.workspace_id = w.id) AS datasets,
                    ...
                    (SELECT COUNT(*) FROM image i LEFT JOIN image_workspace iw on i.id = iw.image_id WHERE (i.access = 1) or (i.access = 0 and iw.workspace_id='{workspace_id}')) AS images
            FROM workspace w
...
def reset_workspace_deployment_instance(workspace_id):
    try:
        with get_db() as conn:
            cur = conn.cursor()
...
def reset_workspace_project_instance(workspace_id):
    try:
        with get_db() as conn:
            cur = conn.cursor()
