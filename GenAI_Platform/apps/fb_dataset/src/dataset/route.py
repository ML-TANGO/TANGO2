...
def create_dataset(workspace_id : Annotated[int, Form()],
    ...
    try:
        ...
        user_name, _ = get_auth()
        res = dataset_svc.create_dataset(body=args, headers_user=user_name)
        if res:
    ...
    # upload_svc.upload_data(files=doc, path=dataset_path, headers_user=user_name)
...
#     res = dataset_svc.update_dataset_info(body = args, headers_user=user_name)
#     return res
...
def preview(args: model.PreviewModel = Depends()):
    user_name, _ = get_auth()
    res = dataset_svc.preview(body=args,headers_user=user_name)
    return res    
...
def decompress(args: model.DecompressModel = Depends()):
    user_name, _ = get_auth()
    res = dataset_svc.decompress(body = args, headers_user=user_name)
    return res
...
#     cr = CustomResource()
#     res = dataset_svc.decompress_check(args,headers_user=user_name)
#     return res
...
def github_upload(args :model.GithubModel):
    user_name, _ = get_auth()
    res = dataset_svc.git_clone(args, headers_user=user_name)
    return res
...
def handle(args :model.CopyModel):
    user_name, _ = get_auth()
    res = dataset_svc.handle_data(args, headers_user=user_name)
    return res
...
#     cr = CustomResource()
#     res = download_svc.download(args,headers_user=user_name)
#     return res

@dataset.get("/{dataset_id}")
def get_dataset_info(dataset_id):
    user_id = get_user_id()
    res = dataset_svc.get_dataset_info(dataset_id=dataset_id,user_id=user_id)
    return res
...
def get_file_list(dataset_id, args : model.DataSearchModel = Depends()):
    ...
    res = dataset_svc.get_dataset_files(dataset_id=dataset_id, search_path=search_path, search_page=search_page, search_size=search_size,
                            search_type=search_type, search_key=search_key, search_value=search_value, sort_type=sort_type, reverse=reverse, headers_user=user_name)
    return res
...
def remove_data(dataset_id,
    ...
    user_name, _ = get_auth()
    res = dataset_svc.remove_data(dataset_id=dataset_id, body=body, headers_user=user_name)
    return res
...
def update_data(dataset_id,
    ...
    user_name, _ = get_auth()
    res = dataset_svc.update_data(body=body, headers_user=user_name)
    return res
...
def makedir(dataset_id,
    ...
    user_name, _ = get_auth()
    res = dataset_svc.make_empty_dir(args, user_name)
    return res
...
def get_dataset_marker(dataset_id, search_path : str = '/'):
    user_name, _ = get_auth()
    res = dataset_svc.get_dataset_marker_files(dataset_id=dataset_id, search_path=search_path, headers_user=user_name)
    return res
...
def delete_dataset(id_list):
    dataset_ids = id_list
    user_name, _ = get_auth()
    res = dataset_svc.remove_dataset(id_list=dataset_ids, headers_user=user_name)
    return res
...
def get_filebrowser(dataset_id):
    user_name, _ = get_auth()
    res= dataset_svc.get_filebrowser_url(dataset_id=dataset_id, headers_user=user_name)
    return res
...
def active_filebrowser(dataset_id, body :model.FileBrowser, background_tasks : BackgroundTasks):
    user_name, _ = get_auth()
    res= dataset_svc.active_filebrowser_pod(dataset_id=dataset_id, active=body.active, background_tasks=background_tasks, headers_user=user_name )
    return res
...
def get_tree(dataset_id):
    user_name, _ = get_auth()
    res= dataset_svc.get_tree(dataset_id=dataset_id, headers_user=user_name)
    return res
# @dataset.post("/import")
# 
...
#     cr = CustomResource()
#     res = dataset_svc.google_drive_upload(args, headers_user=user_name)
#     return res
...
#     cr = CustomResource()
#     res = dataset_svc.upload_ws_dataset(args, headers_user=user_name)
#     return res
