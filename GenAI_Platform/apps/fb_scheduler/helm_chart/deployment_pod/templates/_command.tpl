...
python3 /addlib/history.py --deployment_worker_id {{ .Values.pod.spec.container.command.deploymentWorkerId }} --interval 600 --search_type range --update_time 10 & 
python3 /addlib/history.py --deployment_worker_id {{ .Values.pod.spec.container.command.deploymentWorkerId }} --interval 1 --search_type live --update_time 1 &
{{- end -}}

{{- define "run_code" -}}
cd $JF_DEPLOYMENT_PWD;
echo 'DEPLOYMENT RUNCODE START';
{{ .Values.pod.spec.container.command.runCode }};
{{- end -}}
...
(
    ...
    format=\"+%Y-%m-%d %H:%M:%S\";
    old_start_time=$(cat /log/pod_run_time.json | sed 's/,.*//g'  | sed 's/^{ \"start_time\": //g'  | sed 's/\"//g');                             
