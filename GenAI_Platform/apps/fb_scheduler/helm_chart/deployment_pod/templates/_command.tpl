...
2025-04-24 @author Luke
pod.yaml에 command로 직접 넣게 되니 디버깅이 어려워,
jf-bin에 support/start_worker.sh 실행하는 방식으로 변경.
임시로 기존 코드는 남겨둠.
...
# {{- define "set_pip_repo" }}
#     echo 'Private PIP repo 설정'
#     echo 'export PIP_CONFIG_FILE=/root/.config/acryl-pip/pip.conf' >> $HOME/.bashrc
...
# python3 /addlib/history.py --deployment_worker_id {{ .Values.command.deploymentWorkerId }} --interval 600 --search_type range --update_time 10 & 
# python3 /addlib/history.py --deployment_worker_id {{ .Values.command.deploymentWorkerId }} --interval 1 --search_type live --update_time 1 &
# {{- end -}}
...
# echo 'HUGGINFACE MODLE DOWNCHECK';
# {{- if ne .Values.deployment.model_type "custom" }}
# python3 /addlib/built_in_model_download.py
# {{- end }}
# echo 'DEPLOYMENT RUNCODE START';
# {{ .Values.command.runCode }};
# {{- end -}}
...
#         mem_usage_per=$(awk -v usage=${mem_usage} -v limit=${mem_limit} 'BEGIN { print usage / limit * 100 }');
...
# {{/*
# worker_processes 부분 추가 안함 -> TODO, configmap 따로 적용, 제외
