...
metadata:
  ...
  namespace: {{ .namespace }}
  annotations:
    metallb.universe.tf/loadBalancerIPs: {{ .serverIP | quote }}
    metallb.universe.tf/allow-shared-ip: "acryl-some-key"
  {{- if .annotations }}
...
{{- $autoEnv := dict -}}
  ...
  {{- $_ := set $autoEnv "CONTROLLER_INGRESS_CLASS" .Values.global.developer -}}
  {{- $_ := set $autoEnv "CONTROLLER_ELECTION_ID" (printf "kong-ingress-controller-leader-%s" .Values.global.developer) -}}
...
*/}}
{{- if .Values.global.jfb.cert.enabled }}
{{- $_ := set $autoEnv "KONG_LUA_SSL_TRUSTED_CERTIFICATE" "system" -}}
{{- end }}
